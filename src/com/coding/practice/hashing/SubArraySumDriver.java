...
public class SubArraySumDriver {
    public static void main(String[] args) {
        ...
        //Test case #1 - K at end of array - PASS
//        int[] input = {0,0,0,0,0,0,0,0,0,2};
//        int requiredSum = 2;

        //Test case #2 - K at start of array - PASS
...
//        int requiredSum = 120;
        ...
        //Test case #7 - K - in middle of the array
//        int[] input = {10,20,30,40,50,62,74,83,99,129,318};
//        int requiredSum = 318;

        //Test case #8 - K - in middle of the array
...
//        System.out.println("==================");
        ...
        int[] startEndIndices = fetchStartEndIndices(requiredSum, inputSize, prefixSum);

        if(null == startEndIndices){
            return new int[]{-1};
        }

        if(startEndIndices.length == 1 ){
            return new int[]{input[0]};
        }

        start = startEndIndices[0];
        end = startEndIndices[1] + 1;
       ...
       // System.out.println(" Start : " + start + " end : " + end);
        ...
        for(int loopIndex = 0; loopIndex <= input.length; loopIndex++){
            if(start < end){
                subArray[loopIndex] = input[start++];
            }
        }
    ...
    private int[] fetchStartEndIndices(int requiredSum, int inputSize, int[] prefixSum) {
        ...
        for(int loopIndex = 0; loopIndex < inputSize; loopIndex++){
            ...
            if(prefixSumDifference == 0){
                System.out.println();
                if(null == prefixSumDifferenceSet.get(prefixSumDifference)) {
                    startEndFlags[0] = 0;
                    startEndFlags[1] = loopIndex;
                }
                else {
                    startEndFlags[0] = 0;
                    startEndFlags[1] = prefixSumDifferenceSet.get(prefixSumDifference)[0];
                }
                return startEndFlags;
            }
            if(null != prefixSumDifferenceSet.get(prefixSumDifference)){
                startEndFlags = prefixSumDifferenceSet.get(prefixSumDifference);
                startEndFlags[1] = loopIndex;
...
//                System.out.println("(loopIndex) : " + startEndFlags[1]);
                return prefixSumDifferenceSet.get(prefixSumDifference);
            }
            else {
                startEndFlags[0] = (loopIndex + 1);
//                System.out.println("Absent in set ");
                prefixSumDifferenceSet.put(prefixSum[loopIndex], startEndFlags);
//                System.out.println("prefixSumDifference : " + prefixSumDifference);
//                System.out.println("prefixSum[loopIndex] : " + prefixSum[loopIndex]);
//                System.out.println("input[loopIndex] : " + input[loopIndex]);
//                System.out.println("(loopIndex + 1) : " + startEndFlags[0]);
//                System.out.println("================== \n");
            }
        }
        return null;
